...
class _Reach(EnvWithAdditions):
    ...
    def __init__(self, dt=100, anti=True, rewards=None, timing=None,
        ...
        self.seq = seq
        self.dt = dt    
        #self.rule_input_dur = kwargs['rule_input_dur']
        ...
        self.timing = {
            'fixation': 500,
            'stimulus': 500,
            'rule': 0, # get an error if not present, but the timing is set in add_period
            'delay' : 500,
            'decision': 500}
        if not dly:
        ...
        name = {'fixation': 0, 'stimulus': range(1, dim_ring + 1), 'rule': dim_ring+2} # add rule key/val
        self.observation_space = spaces.Box(
            -np.inf, np.inf, shape=(3+dim_ring,), dtype=np.float32, name=name) # expand ob space by 1
    ...
    def _new_trial(self, **kwargs):
        ...
        fix_time = self.timing['fixation'] 
        stim_time = self.timing['stimulus'] 
        delay_time = self.timing['delay'] 
        after = fix_time+stim_time+delay_time
        ...
        if not self.reaction:
            periods = ['fixation', 'stimulus', 'rule', 'delay', 'decision']
            self.add_period(periods)
            self.add_period('rule', after = after, duration=self.rule_input_dur)
            self.add_ob(1, period=['fixation', 'stimulus', 'delay'], where='fixation')
            self.add_ob(stim, 'stimulus', where='stimulus')
            
            self.add_ob(1, period='rule', where='rule')
...
class _DMFamily(EnvWithAdditions):
    ...
    def __init__(self, dt=100, rewards=None, timing=None, sigma=1., cohs=None,
                 ...
                 delaycomparison=True, integrate=0, seq=0, rule_input_dur=100):
        super().__init__(dt=dt)
        ...
        name = {
            ...
            'rule': 2*dim_ring + 2}
        self.observation_space = spaces.Box(
            -np.inf, np.inf, shape=(3 + 2 * dim_ring,),
    ...
    def _new_trial(self, **kwargs):
        ...
        if self.delaycomparison:
            fix_time = self.timing['fixation']
            stim1_time = self.timing['stim1']
            stim2_time = self.timing['stim2']
            delay_time = self.timing['delay']
            after = fix_time + stim1_time + stim2_time + delay_time 
        ...
        self.add_ob(1, 'rule', where='rule')
        self.set_ob(0, 'rule', where='fixation')
...
class _DelayMatch1DResponse(EnvWithAdditions):
    ...
    def __init__(self, dt=100, rewards=None, timing=None, sigma=1.,
                 dim_ring=16, matchto='sample', matchgo=True, integrate=0, seq=0, rule_input_dur=100):
        super().__init__(dt=dt)
        self.integrate = integrate
        self.omega = 0.01 #angular velocity(rad/ms)
        ...
        name = {'fixation': 0, 'stimulus': range(1, dim_ring + 1), 'rule':dim_ring+2}
        self.observation_space = spaces.Box(
            -np.inf, np.inf, shape=(3 + dim_ring,), dtype=np.float32, name=name)
        name = {'fixation': 0, 'choice': range(1, dim_ring + 1)}
        self.action_space = spaces.Discrete(1+dim_ring, name=name)
    ...
    def _new_trial(self, **kwargs):
        ...
        fix_time = self.timing['fixation']
        sample_time = self.timing['sample']
        delay_time = self.timing['delay']
        test_time = self.timing['test']
        after = fix_time + sample_time + delay_time + test_time 
...
from neurogym.wrappers import DelayedRuleInput
